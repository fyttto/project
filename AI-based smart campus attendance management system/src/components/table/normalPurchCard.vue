<template>
  <div class="tableBlock">
    <el-table :data="normalData" style="width: 100%" :row-class-name="tableRowClassName">
...
export default {
  ...
  data() {
    return {
      ...
      normalData: [{ className: "", collegeName: "", gradeName: "", studentNo: "", subjectName: "", username: "", record: { classNo: "" } }],
    };
  },
  mounted() {
    // 挂载时候获取数据
    this.normalData = this.$store.state.normalData;
    this.$forceUpdate();
    console.log(this.normalData);
  },
  wacth: {
    // 实时发现数据变化，渲染视图
    "$store.state.normalData"(newVal, oldVal) {
      this.noemalData = newVal;
      this.$forceUpdate();
    },
  },
  methods: {
